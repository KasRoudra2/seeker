...
import requests
import traceback
from os import path, kill, popen, system
...
parser.add_argument('-t', '--tunneler', help='Index of the tunneler')
parser.add_argument('-o', '--option', help='Index of the template')
...
TUNNELERS_JSON = f'{path_to_script}/template/tunnelers.json'
...
NGROK_MOD = f'{path_to_script}/template/mod_ngrok.py'
CLOUDFLARED_MOD = f'{path_to_script}/template/mod_cloudflared.py'
META_URL = 'https://raw.githubusercontent.com/kasroudra2/seeker/main/metadata.json'
...
def tunneler_select(tunneler):
    print(f'{Y}[!] Select a Tunneler :{W}\n')
     
    with open(TUNNELERS_JSON, 'r') as tunnel:
        tunnel_info = tunnel.read()

    tunnel_json = loads(tunnel_info)
    for item in tunnel_json['tunnelers']:
        name = item['name'].title()
        print(f'{G}[{tunnel_json["tunnelers"].index(item)}] {C}{name}{W}')

    try:
        if tunneler is None:
            selected = int(input(f'{G}[>] {W}'))
        else:
            selected = int(tunneler)
        if selected < 0:
            print()
            print(f'{R}[-] {C}Invalid Input!{W}')
            sys.exit()
    ...
    try:
        new_tunneler = tunnel_json['tunnelers'][selected]['name']
    except IndexError:
        print()
        print(f'{R}[-] {C}Invalid Input!{W}')
        sys.exit()
    ...
    print()
    print(f'{G}[+] {C}Loading {Y}{new_tunneler.title()} {C}Tunneler...{W}')

    module = tunnel_json['tunnelers'][selected]['module']
    if module is True:
        imp_file = tunnel_json['tunnelers'][selected]['import_file']
        importlib.import_module(f'template.{imp_file}')
    else:
        pass
    return new_tunneler
...
def server():
    ...
    with open(LOG_FILE, 'w+') as phplog:
        ...
        try:
            ...
            if php_sc == 200:
                currentport = popen("sed -n 12p template/mod_cloudflared.py | cut -d '=' -f2 | tr -d '\r'").read().strip()
                system(f"sed -i 's+{currentport}+'{port}'+i' {NGROK_MOD}")
                system(f"sed -i 's+{currentport}+'{port}'+i' {CLOUDFLARED_MOD}")
                if preoc:
                    print(f'{C}[ {G}✔{C} ]{W}')
...
try:
    ...
    SERVER_PROC = server()
    TUNNELER = tunneler_select(tunneler)
    wait()
    data_parser()
